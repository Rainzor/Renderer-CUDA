...
extern "C" __global__ void kernel_integrate_dielectric(bool entering_material, CUDASurface<float> lut_directional_albedo) {
...
extern "C" __global__ void kernel_average_dielectric(const CUDASurface<float> lut_directional_albedo, CUDASurface<float> lut_albedo) {
	...
	for (int c = 0; c < LUT_DIELECTRIC_DIM_COS_THETA; c++) {
		float cos_theta = lut_dielectric_map_cos_theta(c);
		float albedo = lut_directional_albedo.get(i, r, c);
		//surf3Dread(&albedo, lut_directional_albedo, i * sizeof(float), r, c, cudaBoundaryModeClamp);

		avg = online_average(avg, albedo * cos_theta, c + 1);
	}

	lut_albedo.set(i, r, 2.0f * avg);
	//surf2Dwrite(2.0f * avg, lut_albedo, i * sizeof(float), r);
}
